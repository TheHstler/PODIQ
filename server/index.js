  ...
  try {
    ...
    const episodes = items.map((item, index) => {
      ...
      if (enclosure && enclosure.getAttribute("url"))
        ...
        const gm = mediaGroup.getElementsByTagName("media:content")[0];
        audioSrc = gm ? gm.getAttribute("url") || "" : "";
      }
...
app.post("/api/transcribe", async (req, res) => {
    ...
    try {
      ...
      if (sentences.length > 0) {
        ...
        const transcriptLines = sentences.map((sentence) => ({
          time: Math.floor((sentence.startTime || 0) / 1000), /* Taddy gives the start time in milliseconds but we need seconds so we divide by 1000, which is my format */
          text: sentence.text,
          speaker: null, /*For taddy it doesnt identify speakers, so its set to null, but the format is still there for Assembly AI to fill in */
          speakerLabel: null,
        }));
      ...
      console.log(
        "[server] Taddy has no transcript — falling back to AssemblyAI",
      );
    } catch (err) {
  ...
  console.log(
    `[server] Transcribing with AssemblyAI (speaker diarization on): ${audioUrl}`,
  );
  ...
  try {
    ...
    const submitResponse = await fetch(
      "https://api.assemblyai.com/v2/transcript",
      {
        ...
        body: JSON.stringify({
          audio_url: audioUrl, /* sends podcast URL to assembly api  */
          speech_models: ["universal-3-pro"] /* AssemblyAI requires array format */ /* required by AssemblyAI updated API */,
    ...
    /* Poll until complete */
    let transcriptData;
    let attempts = 0;  /* ASSEMBLY API doesnt return the transcript immediately, timestamps audio, backend pols api every 3 seconds checking the status until completed ot after 40 attmepts  */
    while (attempts < 40) {
    ...
    /* ── STEP 3: Use Claude to identify real speaker names ──────────────────
       It sends the first 30 lines of transcript to Claude and ask it to
...
async function identifySpeakers(transcriptLines, episodeTitle) {
  ...
  const uniqueSpeakers = [
    ...new Set(transcriptLines.filter((l) => l.speaker).map((l) => l.speaker)),
  ];
  ...
  try {
    ...
    const message = await client.messages.create({
      ...
      messages: [
        {
          role: "user",
          content: `A podcast listener marked this moment as "${type}" at ${mins}:${secs} in "${episodeTitle}": "${text}". Write ONE short sentence (max 20 words) explaining why this moment might be ${type}. No preamble.`,
        },
      ],
    });
...
${transcriptWithTimestamps}`;
    ...
    let chapters = [];
    try {
      chapters = JSON.parse(rawText.replace(/```json|```/g, "").trim());
    } catch (e) {
      chapters = [];
    }
...
app.get("/api/search", async (req, res) => {
  ...
  const TADDY_API_KEY = process.env.TADDY_API_KEY;
  if (!TADDY_USER_ID || !TADDY_API_KEY)
    return res.status(500).json({ error: "Taddy API credentials not set." });
...
app.listen(PORT, () => {
  ...
  console.log(
    `   Taddy API: ${process.env.TADDY_API_KEY ? "YES ✅" : "NO ❌"}`,
  );
  console.log(
    `   AssemblyAI: ${process.env.ASSEMBLYAI_API_KEY ? "YES ✅" : "NO ❌"}`,
  );
  console.log(
